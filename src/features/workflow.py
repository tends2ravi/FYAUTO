...
from .video_preferences import VideoPreferences, FormatSettings, Resolution, DurationRange
...
class VideoProductionWorkflow:
    ...
    def __init__(self, max_retries: int = 3, retry_delay: float = 1.0):
        ...
        self.music_manager = BackgroundMusic(error_handler=self.error_handler)
        ...
        # Track active tasks using a list of TaskWrapper objects
        self.active_tasks: List[TaskWrapper] = []
    ...
    async def create_video(
        ...
        try:
            ...
            tasks = {
                ...
                "captions": asyncio.create_task(self.caption_generator.generate_captions(
                    script_data,
                    frame_size=(format_settings.resolution.width, format_settings.resolution.height)
                )),
            ...
            # Convert results to a stable format
            stable_results = {
                name: result.result for result in results
            }
            
            # Assemble video
            video_path = await self._assemble_video(
                script_data=script_data,
                audio_files=stable_results["audio"],
                visuals=stable_results["visuals"],
                captions=stable_results["captions"],
    ...
    async def _wait_for_tasks(self, tasks: Dict[str, asyncio.Task]) -> List[TaskResult]:
        """Wait for tasks to complete with error handling."""
        results = []
        
        try:
            for name, task in tasks.items():
                task_wrapper = TaskWrapper(name=name, task=task)
                self.active_tasks.append(task_wrapper)
                try:
                    result = await task
                    results.append(TaskResult(name=name, result=result))
                except Exception as e:
                    ...
                    if task_wrapper in self.active_tasks:
                        self.active_tasks.remove(task_wrapper)
            
            return results
            
        except Exception as e:
            # Cancel remaining tasks
            for task_wrapper in self.active_tasks:
                if not task_wrapper.task.done():
    ...
    async def _cleanup(self) -> None:
        ...
        try:
            ...
            await asyncio.gather(*[task_wrapper.task for task_wrapper in self.active_tasks], return_exceptions=True)
